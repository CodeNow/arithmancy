...
module.exports = class MetricEvent {
  constructor (event) {
    const cleanedEvent = MetricEvent._removeNullAndUndefinedKeys(event)
  ...
  static _removeNullAndUndefinedKeys (event) {
    return Object.keys(event).reduce((outObject, currentKey) => {
      if (event[currentKey] !== null && event[currentKey] !== undefined) {
